...
async def access_aws_credentials(
    ...
    aws_env_vars = request.env_vars

    session = session.Session()

    aws_access_key = aws_env_vars.get("AWS_ACCESS_KEY_ID")
    aws_secret_key = aws_env_vars.get("AWS_SECRET_ACCESS_KEY")
    aws_session_token = aws_env_vars.get("AWS_SESSION_TOKEN")
    aws_region = aws_env_vars.get("AWS_REGION")
    aws_profile = aws_env_vars.get("AWS_PROFILE")
...
async def download_from_s3(
    request: S3DownloadFile, session_values: SessionValues, global_options: GlobalOptions
) -> Digest:
    from botocore import auth, compat, exceptions  # pants: no-infer-dep

    complete_env = session_values[CompleteEnvironmentVars]
    logger.debug(f"complete_env: {complete_env}")

    aws_env_vars = {
        name: value
        for name, value in complete_env.get_or_match("AWS_*")
    }
    logger.debug(f"aws_env_vars: {aws_env_vars}")

    aws_credentials = await Get(
        AWSCredentials, 
        AWSCredentialsRequest(env_vars=FrozenDict(aws_env_vars))
    )
    logger.debug(f"aws_credentials: {aws_credentials}")
...
async def download_file_from_path_s3_authority(
    request: DownloadS3AuthorityPathStyleURL
) -> Digest:
    split = urlsplit(request.url)
